...
function processField(item, field, value, rawValue) {
	if (Zotero.Utilities.trim(value) == '') return;
	if (fieldMap[field]) {
	...
	// If they do we don't know which is better so we might as well just take the second one
		...
		const label = eprintIds[eprinttype.trim().toLowerCase()];
		if (!label) return;
		...
		const label = extraIdentifiers[field];
...
function getFieldValue(read) {
	...
	if (read == "{") {
		...
		while (read = Zotero.read(1)) { // eslint-disable-line no-cond-assign
...
function jabrefCollect(arr, func) {
	if (arr == null) { // eslint-disable-line no-eq-null
		return [];
	}
...
function processComment() {
	...
	if (comment.startsWith('jabref-meta: groupstree:')) {
		...
		while (records.length > 0) {
			...
			if (record.data == null) { // eslint-disable-line no-eq-null
			...
			for (var i = 1; i < collectionPath.length; i++) {
				...
				var child = jabrefCollect(collection.children, function (n) { // eslint-disable-line no-loop-func
					return (n.name == path);
				});
				if (child.length != 0) {
					child = child[0];
			...
			if (parentCollection && record.intersection == '1') { // intersection with parent
				collection.children = jabrefCollect(collection.children, function (n) { // eslint-disable-line no-loop-func
...
function beginRecord(type, closeChar) {
	...
	while (dontRead || (read = Zotero.read(1))) {
		...
		// read whitespace
			...
			if (item.itemID == null) { // eslint-disable-line no-eq-null
		...
		while (read = Zotero.read(1)) { // eslint-disable-line no-cond-assign
			if (read == "@") {
				...
				if (type == "comment") {
					...
					const maybePromise = beginRecord(type, "}");
					if (maybePromise) {
...
function escapeSpecialCharacters(str) {
	...
	if (newStr.includes('\\vphantom')) {
		var m;
		while (m = vphantomRe.exec(newStr)) { // eslint-disable-line no-cond-assign
...
var exports = { // eslint-disable-line no-unused-vars
	doExport: doExport,
	doImport: doImport,
